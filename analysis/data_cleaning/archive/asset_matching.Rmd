...
new_assets <- read_csv("data/final_data/Asset Form Data_missing_rows.csv",
                       na = c("", ".", "-99"),
...
cnics_not_in_scrutiny_corrections <- 
  read_excel("data/cnics_not_in_scrutiny_corrections.xlsx",
...
second_uid_correct <- read_xlsx("data/final_data/uid_correction_second.xlsx") %>%
...
check_constituency("00101", "PS-67")
dup_uids %>%
  filter(length(unique(constituency_code)) > 1 | length(unique(candidate_name)) > 1) %>%
  as.data.frame
...
write_csv(left_join(res_u, matched_keys), "data/master_res_match.csv")
write_csv(cleaned_assets, "data/master_cleaned_assets.csv")
